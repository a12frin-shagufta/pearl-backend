...
const readVariantStocks = (req, colorArrayLength) => {
  ...
  if (variantStocksFromJson && variantStocksFromJson.length > 0) {
    for (
      let i = 0;
      i < Math.min(colorArrayLength, variantStocksFromJson.length);
      i++
    ) {
...
const addProduct = async (req, res) => {
  try {
    ...
    for (let i = 0; i < colorArray.length; i++) {
      ...
      if (imageFile) {
        ...
        try {
          ...
          throw new Error(
            `Image upload failed for color "${color}": ${error.message}`
          );
        }
      }
      ...
      if (videoFile) {
        ...
        try {
          ...
          throw new Error(
            `Video processing failed for color "${color}": ${error.message}`
          );
        }
      }
    ...
    for (let i = 0; i < colorArray.length; i++) {
      ...
      for (const video of videos) {
        ...
        if (typeof video === "string") {
          console.error(
            `❌ REJECTED: Video stored as STRING for "${color}": "${video.substring(
              0,
              50
            )}..."`
          );
          throw new Error(
            `VIDEO_ERROR: Video for "${color}" stored as string.`
          );
        }
        ...
        if (!video?.key || !video?.signedUrl) {
          console.error(
            `❌ REJECTED: Missing key/signedUrl for "${color}":`,
            video
          );
          throw new Error(
            `VIDEO_ERROR: Video for "${color}" missing key or signedUrl.`
          );
        }
    ...
    console.log(
      `🎉 Product "${name}" added successfully with ${colorArray.length} variant(s)`
    );
...
const updateProduct = async (req, res) => {
  try {
    ...
    if (parsedColors.length > 0) {
      ...
      for (let i = 0; i < parsedColors.length; i++) {
        ...
        if (vidFile) {
          ...
          console.log(
            `  🎥 variantVideo${i}: "${vidFile.originalname}" → ${sizeMB}MB`
          );
        }
        if (imgFile) {
          console.log(
            `  🖼️ variantImage${i}: "${imgFile.originalname}" → ${(
              imgFile.size /
              1024 /
              1024
            ).toFixed(1)}MB`
      ...
      if (variantVideoEntries.length > 0) {
        ...
        for (let i = 0; i < variantVideoEntries.length; i++) {
          ...
          console.log(
            `📤 Video ${entry.index}: "${entry.file.originalname}" (${sizeMB}MB)`
          );
          ...
          try {
            ...
            const existingMatch = existing.variants?.find(
              (v, idx) =>
                idx === entry.index || v.color === parsedColors[entry.index]
            );
            uploadedVideoUrlsForIndex[entry.index] =
      ...
      variants = parsedColors.map((color, i) => {
        ...
        if (existingMatch && existingMatch.videos) {
          if (Array.isArray(existingMatch.videos)) {
            ...
            existingVideos = existingMatch.videos
              .filter((v) => {
                if (Array.isArray(v)) {
      ...
      for (const variant of variants) {
        console.log(
          `  Checking ${variant.videos?.length || 0} videos for ${
            variant.color
          }`
        );
        for (const video of variant.videos || []) {
          if (typeof video === "string") {
            console.error(
              `❌ UPDATE_REJECTED: Video as string for "${
                variant.color
              }": ${video.substring(0, 50)}...`
            );
            throw new Error(
              `UPDATE_ERROR: Video for "${variant.color}" stored as string`
            );
          }
          if (!video?.signedUrl) {
            console.error(
              `❌ UPDATE_REJECTED: Missing signedUrl for "${variant.color}":`,
              video
            );
            throw new Error(
              `UPDATE_ERROR: Video for "${variant.color}" missing signedUrl`
            );
          }
          console.log(
            `  ✅ Video OK for ${variant.color}: ${video.key?.substring(
              0,
              30
            )}...`
          );
    ...
    console.log(
      `✅ Updated product ${id}: ${
        parsedColors.length || existing.variants?.length || 0
      } variants`
    );
    res.json({
      success: true,
    ...
    videosArray.map(async (video) => {
      ...
      if (typeof video === "string") {
        if (video.includes("cloudinary.com")) {
          console.warn(
            `⚠️ Skipping Cloudinary URL: ${video.substring(0, 50)}...`
          );
          return null;
        }
        ...
        try {
          console.log(
            `🔗 Converting string to 7-day URL: ${video.substring(0, 30)}...`
          );
      ...
      if (video && typeof video === "object" && video.key) {
        ...
        const expiresIn24h =
          video.expiresAt && video.expiresAt - now < 24 * 60 * 60 * 1000; // 24 hours warning

        console.log(`🔍 Checking: ${video.key.substring(0, 30)}...`);
        console.log(
          `   Status: ${
            isExpired ? "EXPIRED" : expiresIn24h ? "EXPIRES SOON" : "VALID"
          }`
        );
        ...
        if (isExpired || expiresIn24h || !video.signedUrl) {
          console.log(
            `🔄 Refreshing URL (${isExpired ? "expired" : "expiring soon"})...`
          );
          try {
        ...
        console.log(
          `✅ Valid for ${Math.round(
            (video.expiresAt - now) / (1000 * 60 * 60)
          )} more hours`
        );
        return video;
      }
        ...
        if (productObj.variants) {
          for (let i = 0; i < product.variants.length; i++) {
            ...
            if (variant.videos && variant.videos.length > 0) {
              console.log(
                `🎬 Processing ${variant.videos.length} videos for ${product.name}`
              );

              try {
                const originalVideos = [...variant.videos];
                const refreshedVideos = await generateFreshVideoUrls(
                  variant.videos
                );
                ...
                const wasRefreshed = refreshedVideos.some(
                  (v, idx) =>
                    v.refreshedAt ||
                    v.previouslyExpired ||
                    (originalVideos[idx] &&
                      originalVideos[idx].signedUrl !== v.signedUrl)
                );
                ...
                if (wasRefreshed) {
                  ...
                  needsDbUpdate = true;
                  console.log(
                    `🔄 URLs refreshed for ${product.name} - ${variant.color}`
                  );
                }
              } catch (err) {
                console.error(
                  `❌ Video processing failed for ${product.name}:`,
                  err.message
                );
    ...
    if (productObj.variants && productObj.variants.length > 0) {
      for (const variant of productObj.variants) {
        if (variant.videos && variant.videos.length > 0) {
          ...
          const hasOldFormat = variant.videos.some(
            (v) => typeof v === "string"
          );
          const hasNewFormat = variant.videos.some(
            (v) => v && typeof v === "object" && v.key
          );
...
const decrementStock = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Stock updated.", stock: newStock });
  } catch (err) {
            ...
            if (typeof video === "string") {
              ...
              variantInfo.videos.push({
                ...
                signedUrlPreview: video.signedUrl
                  ? "..." +
                    video.signedUrl.substring(video.signedUrl.length - 20)
                  : null,
                hasExpiresAt: !!video.expiresAt,
                expiresAt: video.expiresAt,
                isExpired: video.expiresAt
                  ? video.expiresAt < Date.now()
                  : null,
              });
            } else {
              variantInfo.videos.push({
